			...
			image = ImageIO.read(f);
				...
				for (int i = 0; i < width - 1; i++) {
					...
					for (int j = 0; j < height - 1; j++) {
							...
							int a = (foto[i][j]>>24) & 0x000000ff; //alpha
							int r = (foto[i][j]>>16) & 0x000000ff; //red
							int g = (foto[i][j]>>8) & 0x000000ff; //red
							int b =  foto[i][j] & 0x000000ff; //red
							//xs.add(i);
							//ys.add(j);
			//				System.out.println();
